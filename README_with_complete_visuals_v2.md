...
The implementation orders the eigenvectors by decreasing eigenvalue and applies a deterministic sign convention so that repeated runs produce stable axis directions. The crown-versus-root polarity is resolved separately from the arbitrary mathematical sign of an eigenvector.

#### PCA is the starting point — not the complete anatomical decision

For molars, the direction of maximum variance is not always sufficient to define the anatomical crown-to-root direction: the crown is wide and the roots can splay. The implementation therefore evaluates the principal directions in the context of the tooth topology and selects the direction that best separates the wide crown end from the narrower root-apex end.
...
A single noisy cross-section is not sufficient; the implementation requires persistent component separation to reduce sensitivity to segmentation noise.
...
The project explicitly documents that the physical-coordinate transformation is essential because anisotropic spacing can otherwise distort the recovered direction.
...
The application provides a browser-based interface for running the complete pipeline without writing code. The screenshots below show a real end-to-end analysis run, from CBCT upload to quantitative transverse measurements and visual verification.

### Interface Preview

#### 1. CBCT Upload and Analysis Configuration

![Streamlit CBCT upload interface](docs/images/interface-upload.png)

**Figure 3 — CBCT upload and configuration.** The user selects the compute device and nnU-Net fold, configures diagnostic cut-offs if needed, uploads a NIfTI or DICOM study, and starts the analysis from the Streamlit interface.

#### 2. Predicted Transverse Widths and Skeletal Classification

![Predicted transverse widths and diagnosis](docs/images/interface-results.png)

**Figure 4 — Quantitative results dashboard.** The interface reports the predicted maxillary and mandibular widths, compares each arch with the configured normal range, calculates the Yonsei Transverse Index, and displays the resulting transverse skeletal classification and sub-class.

#### 3. CR-Based Measurement Visualization

![Axial visualization of CR-based transverse width measurement](docs/images/interface-measurement.png)

**Figure 5 — Measurement visualization.** Axial CBCT slices are shown at the estimated furcation/center-of-resistance level. The first-molar CR points are highlighted and the yellow line represents the bilateral transverse width used for the final maxillary or mandibular measurement.

#### 4. Interactive 3D Molar and Furcation Visualization

![Interactive 3D molar mesh with estimated furcation point and long axis](docs/images/interface-3d-molar-cr.png)

**Figure 6 — Interactive 3D tooth visualization.** The segmented first molar is rendered as a rotatable 3D surface mesh. The estimated pipeline furcation/center-of-resistance point is displayed directly on the tooth geometry, together with the recovered long axis and physical coordinates in millimetres. This view provides a direct visual sanity check of the geometric localization result on the segmented anatomy.

#### 5. Furcation Localization Diagnostics

![Furcation localization diagnostic visualization](docs/images/furcation-localization-diagnostics.png)

**Figure 7 — Furcation localization diagnostics.** This diagnostic view exposes the internal geometric reasoning of the first-molar analysis. Cross-sections are sampled along the recovered crown-to-root axis and the number of connected root components is tracked at each position. The highlighted slice marks the selected three-root furcation configuration for a maxillary first molar. The lower-left panel shows the convex-hull webbing region used to calculate its centroid, while the lower-middle panel back-projects the estimated point into the tooth coordinate frame and compares it with the 3D skeleton cross-check. The metadata panel reports the expected and detected root counts, selected rule, voxel spacing, furcation depth, center-of-resistance coordinates, and confidence information.

This visualization is especially useful for **explainability and debugging**: instead of returning only a final coordinate, the pipeline shows why a particular furcation plane was selected and how the resulting center-of-resistance estimate was obtained.

#### 6. Bilateral Transverse Width From CR Points

![Maxillary transverse width measured between bilateral first-molar center-of-resistance points](docs/images/transverse-width-cr-points.png)

**Figure 8 — Bilateral maxillary transverse-width measurement.** The segmented right and left maxillary first molars are shown in physical patient coordinates, with the estimated center-of-resistance/furcation point marked on each tooth. The line joining the two CR points is the transverse measurement used by the pipeline; in this example the maxillary width is **39.56 mm**. Displaying the tooth masks, tooth labels, CR landmarks, and the final inter-CR distance in the same coordinate frame makes the measurement directly traceable to the underlying 3D anatomy.

The screenshots demonstrate that the project is not only an offline research pipeline: segmentation, geometric analysis, 3D localization, measurements, classification, diagnostic inspection, and visualization are exposed through an interactive application suitable for live demonstration.
...
- Furcation-localization diagnostic view
